...
fn main() {
    ...
    if let Ok(url) = std::env::var("JZ_NODE_RPC") {
        config.rpc_url = url;
    }
    if let Ok(token) = std::env::var("JZ_RPC_TOKEN") {
        config.token = Some(token);
    }
    if let Ok(object_id) = std::env::var("JZ_SYNC_OBJECT") {
        config.object_id = Some(object_id);
    }
    if let Ok(ticks) = std::env::var("JZ_SYNC_TICKS").map(|v| v.parse::<u32>().unwrap_or(60)) {
